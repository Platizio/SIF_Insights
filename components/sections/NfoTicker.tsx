...
import type { Nfo } from "@/lib/data/types";
import { nfoStatus } from "@/lib/format";
import { DUR, EASE } from "@/lib/motion";
import { useTodayIso } from "@/lib/use-today";

/**
 * Section 0 — the live NFO ticker, the client half of <NfoBar>.
 *
 ...
 *
 * The offers arrive as PROPS from the server component in NfoBar.tsx. This
 * file used to import `activeNfos` from `@/lib/data` itself, and because it
 * sits in the root layout that one import put the entire NAV history into the
 * chunk every page on the site loads.
 */

/** Only what the strip renders and re-checks — the RSC payload stays small. */
export type TickerNfo = Pick<Nfo, "id" | "title" | "active" | "opensOn" | "closesOn">;
 ...
 *
 * `nfos` is every offer that was open at build time plus every announced one
 * with a stated window, so the strip can both lose an offer that closes while
 * a page sits open and gain one that opens before the next deploy. Static
 * rendering makes the build's answer only as fresh as the last deploy; this
 * narrows it again on the client.
 *
 * Hydration stays simple because `useTodayIso` returns `builtOn` during SSR
 * and the first hydrating render, so pass one reproduces the server's markup
 * exactly and the reader's own date lands on the pass after, as an ordinary
 * update rather than a mismatch. Reading `new Date()` straight into render
 * would make the two passes disagree the moment a window opened or shut
 * between build and visit. See lib/use-today.ts for the re-check cadence.
 */
function useOpenNfos(nfos: TickerNfo[], builtOn: string): TickerNfo[] {
  const today = useTodayIso(builtOn);
  return useMemo(() => nfos.filter((nfo) => nfoStatus(nfo, today) === "open"), [nfos, today]);
}

export function NfoTicker({ nfos, builtOn }: { nfos: TickerNfo[]; builtOn: string }) {
  const [dismissed, setDismissed] = useState(false);
  const openNfos = useOpenNfos(nfos, builtOn);
  const { trackRef, trackStyle } = useMarquee(!dismissed && openNfos.length > 0);
 ...
 * invisible.
 *
 * Takes the list as a prop rather than filtering its own, so both copies are
 * guaranteed to render the same items as each other and as the length check
 * above — the marquee's whole -50% loop depends on the two copies being
 * identical.
 */
function TrackCopy({ items, duplicate = false }: { items: TickerNfo[]; duplicate?: boolean }) {
  return (
 ...
 *
 * `mounted` re-runs the setup when the track appears. The strip renders
 * nothing while no offer is open, so a page built with none open that then
 * sees one open on the reader's clock mounts the track AFTER the first
 * effect ran — without this it would never be measured or observed.
 */
function useMarquee(mounted: boolean) {
  ...
  useEffect(() => {
    ...
    // This is also the re-measure path when an offer opens or expires
    // mid-session and the item count changes: `.marquee-track` is
    // `width: max-content`, so the track's own box resizes and the observer
    // fires. The two-copy invariant survives because both copies render the
    // same list.
